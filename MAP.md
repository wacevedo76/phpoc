...
|---|---|---|---|
| `main.py` | 472 | HOT | CLI entry — argparse dispatch to all commands, `_handle_config_command`, `_config_generate_template` — wired `--comment`/`-m` and `--show-comments`/`--show-tags` |
| `cli/interface.py` | 450 | HOT | Display: `view_active`, `show_rep`, `list_habits`, `_print_entry`, `_resolve_date_filters` — added `--comment`/`-m`, `--show-comments`/`--show-tags` support |
