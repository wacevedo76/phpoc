...
| **History screen** (staging vs committed + calendar) | N/A | ✅ **DONE** — History screen shows completed entries from real SyncService with status badges (Not Committed / Committed). Expand/collapse tags & comments on card click. Red border for staging (blue when expanded). Staging entries can add/remove tags and edit comments inline via × buttons, +input, and textarea. No commit UI. Custom month calendar widget with year/month navigation, entry-dot indicators, click-to-filter date. Committed entries decrypted from `ledger:blocks` via `_rawCommittedEntryToDTO()`. See `SESSION_HANDOFF.md` Steps 8, 16. |
| **Data Management** (Settings import/export) | N/A | ✅ **DONE** — Export (auth-gated PassphraseModal) and Import (file + seed + passphrase) in Settings screen. Full ledger export via `exportLedgerFull()` exports committed chain + staging (v2 format, 72 tests). Import handles three formats: v1 export, v2 export, raw CLI chain. Two-phase import with destroy warning + staging persistence. Cross-platform Python-compatible JSON serializer for block seal / entry hash verification on raw chain imports. See `PHPOC-REACT_WEB-DESIGN_DECISIONS.md` §11.11.
...
| 32 | **Sync Screen Pause Management** — `PausesEditor` component beneath end-time/duration on expanded stopped entries. Lists existing pauses with start/stop times and duration badge. × button removes each pause. "+ Add pause" opens inline form with start/end time inputs + Save/Cancel. Pauses are inserted sorted by start time. Recalculates duration on every pause change. | 0.5 day | ✅ Done | Jun 16 2026 |
| 33 | **Sync Screen Enter-Key Fix** — Fixed bug where pressing Enter in the tag-add input collapsed the entire card. Root cause: React keydown event bubbled from the input to the card's `onKeyDown` handler which toggled expand/collapse. Fix: `e.stopPropagation()` in `handleTagInputKeyDown` + `onKeyDown={(e) => e.stopPropagation()}` on the expanded details wrapper div. | 0.1 day | ✅ Done | Jun 16 2026 |
| 34 | **Duplicate Entry Race Condition Fix** — Fixed read-modify-write race in `LocalCache.update()` that caused committed entries to lose their `committed: true` flag when inline edits (end-time, duration, pauses, tags, comments) raced with `markCommitted()`. Fix: `update()` now re-reads from storage immediately before writing, applies changes to the fresh state, and skips the write if the entry was committed or replaced between reads. Three guards: early committed check, index-out-of-range check, entry_id + committed race check. | 0.25 day | ✅ Done | Jun 16 2026 |
| 35 | **Sync Screen Delete-From-Staging Button** — Expanded stopped entries show "🗑 Delete from staging" button that calls `sync.remove()` to delete the entry from staging. Immediate UI update with all editing/selection state cleaned up. | 0.1 day | ✅ Done | Jun 16 2026 |
| 28 | **Full Ledger Export + Import Interface** — `exportLedgerFull(blocks, staging, crypto, masterKey)` exports committed chain + staging in v2 format. HMAC seal over {ledger, staging}. Pure read — never commits. 72 tests with real mock ledger data. Import handles v1/v2 formats + raw CLI chain (JSON array of blocks). Returns genesisHash. Two-phase import: `validateImport()` (read-only) + `confirmImport()` (destructive). | 1 day | ✅ Done | Jun 11 2026 |
| 29 | **Import Workflow Enhancement** — Five read-only validation gates before any destructive write. Destroy warning + export offer in confirmation dialog. Staging persistence checkbox (keep uncommitted entries merged into imported staging). Fixed v2 committed chain loss. Fixed raw chain format detection with Python-compatible JSON serializer (`jsonDumps`) for cross-platform hash compatibility. All 98 import/export tests pass. | 1 day | ✅ Done | Jun 11 2026 |
| 30 | **History Calendar Widget + Committed Entry Decryption** — Replaced `<input type="date">` with custom inline month calendar (year/month nav, day grid with entry-dot indicators, today highlight, click-to-filter). Extended `sync.getCompleted()` to decrypt committed entries from `ledger:blocks` via new `_rawCommittedEntryToDTO()` (AES-128-CTR field decryption vs staging `plain:` convention). Calendar dots and date filtering now work across all committed + staging entries. | 0.5 day | ✅ Done | Jun 11 2026 |
...
| 21 | **History staging/committed differentiation** — History screen shows status badges (Not Committed / Committed), expand/collapse tags & comments on card click, red border for staging (blue when expanded). `StagingEntry` tracks `committed` + `block_index`. `commit()` returns entry IDs for tracking. Tests: 269. | 1 day | ✅ Done | Jun 11 2026 |
| 22 | **Inline tag & comment editing** — staging entries in History can add/remove tags (× button, +input with Enter) and edit comments (textarea with debounced auto-save). Committed entries read-only. | 1 day | ✅ Done | Jun 11 2026 |
...
| 36 | **Ledger Merge — GREEN phase** — `LedgerMerge.merge()` implemented in `src/ledger/merge.js`. Standalone module, 7-step algorithm (fork detection, entry extraction, content_hash dedup, alphabetical sort, chain rebuild with summary inserts, index rebuild). Input chain validation (`_verifyChain`/`_verifyBlockData`) runs as Step 0 before fork detection — validates each chain independently (seal, prev_hash linkage, entry hashes, optional signatures). Short-circuits on first failure. Ordering tests (J8-J10) prove validation fires before genesis mismatch check. 99 assertions, 0 failures. | 0.5 day | ✅ GREEN | Jun 19 2026 |
| 36 | **Remote Sync Wiring — phpoc-web ↔ Cloudflare Worker** — Wire phpoc-web to use the Cloudflare Worker as a remote backend. Dual-backend model (local IndexedDB + remote HttpTransport), device UUID persistence (random UUID4 per device, not HMAC-derived), ledger merge strategy (GREEN with input validation + ordering tests), Settings UI with genesis compatibility gate. Architecture decisions in `SESSION_HANDOFF.md`. | 2-3 days | 🔴 Steps 1-6 DONE. **⏭ Next: Step 7 — Settings service dropdown with genesis compatibility gate.** Pull genesis from remote, compare hashes, use LedgerMerge for same-genesis divergent chains, block connection for different genesis. | Jun 18-19 2026 |
...
| `test/device_uuid_test.mjs` | Device UUID generation, IndexedDB persistence, survives refresh/re-login, not derived from master key | P1 | ✅ 22 tests, all passing — `device_uuid.js` implemented and wired (2026-06-18) |
| `test/remote_transport_test.mjs` | `createRemoteTransport()`: null for standalone/mock/memory, HttpTransport for saas/lan with baseUrl, null fallback without baseUrl. Instance isolation, fresh ETag cache, HttpTransport validation passthrough. Architecture: createRemoteTransport (transport only, no storage). | P2 | ✅ GREEN — 40 assertions, 0 failures. `createRemoteTransport()` implemented in `plugin_factory.js` (2026-06-18). |
| `test/remote_config_test.mjs` | localStorage persistence for deployment, baseUrl, apiKey; URL param priority; auto-detect saas from worker URL; fallback to standalone on invalid config; config mutation detection | P2 | ✅ 35 tests, all passing — `detectDeployment()` already exists and passes |

**Device UUID bug:** ✅ **FIXED (2026-06-18).** The WASM `get_device_id(MK)` returns `HMAC(MK, "device:id")` — deterministic from passphrase (per PHPSPEC §2.8 default). The CLI uses `RandomUUIDDeviceIdentityProvider` which generates and persists a random UUID4. If the web app uses WASM's default, all devices with the same passphrase produce the same device UUID, causing the cookie mechanism to incorrectly treat different devices as the same device (Case A — push only, no merge). Fix: `getOrCreateDeviceUuid(storage)` in `src/sync/device_uuid.js` generates `crypto.randomUUID()` on first boot, persists in IndexedDB, and auto-migrates WASM-derived hex UUIDs. `SyncService._getDeviceId()` now reads from storage first, falls back to WASM only as last resort. **Follow-up fix (2026-06-18):** `_reconcileAndClaim()` cookie creation (line 529) was still using `this._crypto.getDeviceId(masterKeyHex)` — the WASM-derived UUID — to populate the remote cookie's `device_uuid` field. Changed to `await this._getDeviceId()` so the remote cookie stores the same per-device UUID4 used for Case A/B matching. Without this, the remote cookie always held a WASM UUID while the local device had a UUID4, causing permanent Case B (pull+merge+push) on every sync. Removed unused `isWasmDerivedUuid` import from sync.js. 122 tests (22 + 40 + 60) all passing.
