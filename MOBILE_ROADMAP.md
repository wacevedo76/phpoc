...
| **History screen** (staging vs committed) | N/A | ✅ **DONE** — History screen shows completed entries from real SyncService with status badges (Not Committed / Committed). Expand/collapse tags & comments on card click. Red border for staging (blue when expanded). Staging entries can add/remove tags and edit comments inline via × buttons, +input, and textarea. No commit UI — commit flow moves to dedicated Sync screen. See `SESSION_HANDOFF.md` Step 8. |
...
4. ✅ **Browser import/export via File API** — `exportLedger()`, `importLedger()`, `PassphraseModal`. Auth-gated (passphrase → `crypto.authenticate()` → master key). Entries sealed via HMAC-SHA256 (`crypto.seal`/`crypto.verifySeal`). Single `.json` file with `{ format_version, exported_at, entries, seal }`. Import verifies seal + re-validates entry hashes. 3 source files, 83 tests. Commit: `0faf316`.
...
| 26 | **Sync Screen with Commit UI** — dedicated Sync screen replacing old sync status panel. Shows all uncommitted entries (active + stopped) in compact cards. Stopped entries: yellow border/left indicator (ready to sync), expandable inline tag & comment editing with × remove, +input add, debounced comment textarea. Active entries: red border (not syncable), compact non-expandable. Commit Selected/Commit All buttons between entries and sync status section. | 0.5 day | ✅ Done | Jun 11 2026 |
