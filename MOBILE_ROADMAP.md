...
| 36 | **Remote Sync Wiring — phpoc-web ↔ Cloudflare Worker** — Wire phpoc-web to use the Cloudflare Worker as a remote backend. Dual-backend model (local IndexedDB + remote HttpTransport), device UUID persistence (random UUID4 per device, not HMAC-derived), Settings UI with genesis compatibility gate, ledger merge strategy. Architecture decisions in `SESSION_HANDOFF.md`. | 2-3 days | 🔜 Steps 1-5 DONE. **Next: Step 6** — Settings service dropdown with genesis gate (G₁≠G₂ block, G₁=G₂ allow), then Step 6b — ledger merge for divergent same-genesis chains. | Jun 18 2026 |
