...
if (typeof exportLedger === 'function') {
  ...
  // Test 3: Entries match input
  assertDeepEq(parsed.entries, SAMPLE_ENTRIES, 'exported entries match input');
...
if (typeof exportLedger === 'function') {
  ...
  // Test 10: Entry hashes are preserved exactly
  for (let i = 0; i < SAMPLE_ENTRIES.length; i++) {
    assertEq(parsed.entries[i].hash, SAMPLE_ENTRIES[i].hash,
      `entry[${i}] hash preserved: ${SAMPLE_ENTRIES[i].title}`);
  }
}
