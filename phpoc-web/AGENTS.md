...
- `src/components/screens/` — Screen components: Auth, Configuration, Dashboard, History, Landing, LedgerSync, NewTask, Onboarding, Settings, SyncSettings, Tags, UserProfile, **CommonplaceScreen** (Commonplace book surface — Slice 3), **AddEntrySheet** (add-not-in-place capture — Slice 3), **TopicIndex** (tag/topic filter chips — Slice 3)
