...
| 17 | **Ledger Merge — TDD GREEN** — `LedgerMerge.merge()` implemented in `src/ledger/merge.js`. Covers fork detection, dedup, summary blocks, alphabetical ordering, chain integrity, index rebuild, stats, edge cases, input chain validation (`_verifyChain`), and ordering tests (J8-J10). 99 assertions, 0 failures. Standalone 7-step algorithm per §11.31 with Step 0 input validation that runs before genesis mismatch check. | ✅ GREEN | Jun 19 2026 |
...
| `phpoc-web/src/ledger/merge.js` | **NEW (2026-06-19)** — `LedgerMerge.merge()` standalone module. 7-step merge algorithm + Step 0 input chain validation (`_verifyChain`/`_verifyBlockData`). Validates each chain independently: seal integrity, prev_hash linkage, entry hashes, optional identity signatures. Short-circuits on first failure. Uses `jsonSort` from utils for consistency with `LedgerChain`. 99 tests green. |
...
> **⏭ Next session resume:** Step 7 — Settings genesis compatibility gate. LedgerMerge is fully GREEN (99 assertions, 0 failures) with input chain validation (`_verifyChain`, `_verifyBlockData`) and ordering tests proving step 0 validation runs before genesis check. Wire merge into the Settings UI: pull genesis from remote, compare hashes, use merge for same-genesis divergent chains, block connection for different genesis.
...
6. ✅ **Ledger merge strategy** — TDD GREEN phase **COMPLETE (2026-06-19)**. `LedgerMerge.merge()` implemented in `src/ledger/merge.js`. Standalone module with 7-step algorithm + Step 0 input chain validation per `PHPOC-REACT_WEB-DESIGN_DECISIONS.md` §11.31. **99 assertions, 0 failures.**

   **Execution order (proven by Group J tests):**
   ```
   Step 0 → _verifyChain('local')     ← J6/J8/J10: throws 'local chain validation failed'
   Step 0 → _verifyChain('remote')    ← J7/J9: throws 'remote chain validation failed'
   Step 1 → genesis mismatch check     ← I2: throws 'Genesis block mismatch'
   Steps 2-7 → merge workflow          ← only reached if all gates pass
   ```

   **Validation checks per block:** seal (HMAC of sorted keys), prev_hash linkage, entry hashes (SHA-256), optional identity signatures. Uses `jsonSort` from utils (recursive key sorting, matching `LedgerChain`). Short-circuits on first failure — local error prevents remote check (J8).  Fork detection walks both chains forward; content_hash dedup uses Set from all local entries; alphabetical sort by title per §11.30; chain rebuild inserts summary blocks via YearMonthSummaryPolicy; only rebuilds when unique remote entries exist (local chain used as-is for subset/identical cases). **Test infrastructure bug fixed:** `buildDayBlock` seal computation in `ledger_merge_test.mjs` now uses proper sorted-key JSON instead of `JSON.stringify(obj, replacerArray)` which acted as a property whitelist, stripping nested entry data and making all same-position blocks appear identical.
...
- **Ledger merge — GREEN (2026-06-19):** `LedgerMerge.merge()` implemented in `src/ledger/merge.js`. Standalone module, 7-step algorithm + Step 0 input chain validation (`_verifyChain`, `_verifyBlockData`). 99 assertions, 0 failures. Validation checks seal, prev_hash linkage, entry hashes, optional signatures per block — mirrors `LedgerChain._verifyBlockData()` on raw arrays. Execution order proven by Group J tests: validation fires before genesis mismatch check; local failure short-circuits before remote check. Seal computation unified with `jsonSort` from utils (recursive sort, matching `LedgerChain`). ⏭ Next: Wire into genesis compatibility gate (Step 7).
