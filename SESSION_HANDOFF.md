...
### 🔨 Wire `dropLedgerCommitted` into the handoff reconcile (ADR-030 Scenario-5/6 follow-on)
- **Context:** Phase 4 REFACTOR of ADR-030 is complete (see below). Review surfaced that the L3
  `dropLedgerCommitted` filter is unit-tested (L3.1/L3.2) but **not wired** into `SyncService._reconcileAndClaimRowLevel`.
  `_pushStagingRowsToRemote` already excludes committed rows (R4), so the practical gap: a fresh device whose
  local scratchpad carries an `activity_id` already in the local ledger would re-push it as scratchpad.
- **Close:** build `Set<String>` of ledger `entry_id`s from `LedgerEngine.getAllBlocks()`; call
  `MergeEngine.dropLedgerCommitted(local, ids)` before push in the handoff reconcile; add an integration assertion
  in `ledger_auto_pull_on_reauth_test.dart`. Tracked: `docs/planning/BACKLOG.md`.
...
- **ADR-030 Scenario-5/6 gap (NEW, not wired):** `MergeEngine.dropLedgerCommitted` is unit-tested (L3.1/L3.2) but NOT invoked in the handoff reconcile — a fresh device could re-push a local scratchpad row whose `activity_id` is already in the local ledger. Tracked in BACKLOG + Immediate Next Steps.
