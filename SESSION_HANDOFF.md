...
| 🥇 Highest | **Phase 4 — Staging Interaction Flow** — ✅ DONE | Wire every-command sync with 500ms timeout and offline tolerance into the staging interaction flow. Created `core/sync/` package (decision, transport, orchestrator) + wired mutations. 69 tests, 779 total. | Done |
...
- 2026-05-14 — Added `--dir` CLI flag (all commands), wired `storage.data_dir` config key into `_resolve_data_dir()`, added 6 new tests (941 total). Priority chain: `--dir` > `$PHPOC_DATA_DIR` > `config storage.data_dir` > XDG default > legacy. (`main.py`, `storage/implementations/file_config.py`, `tests/test_phase7_config_integration.py`)
- 2026-05-14 — Fixed two bugs preventing ledger verify of user data:
  1. `CryptoManager.decrypt()` raised auth tag mismatch for identity secret encrypted pre-R1 (88 bytes ambiguous between old-format-without-tag and new-format-with-tag). Changed tag mismatch to fall back to old format instead of raising.
  2. `LedgerChain._verify_content_hash()` legacy fallback used raw encrypted field values instead of decrypting them, producing wrong content hashes. Changed to decrypt _enc fields in the legacy path.
  (`security/crypto.py` — marked HOT, `domain/ledger/chain.py` — marked HOT)
...
- **2026-05-20**: Added `--comment`/`-m` flag to `add oneoff/start/end`, `--show-comments`/`-c` and `--show-tags` flags to `view`/`list all/synced/staged`. Touched `main.py`, `cli/interface.py`. Annotated `cli/interface.py` as HOT.
