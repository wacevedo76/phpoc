...
6. 🔜 **Settings service dropdown** — Replace hardcoded "Worker URL" + "API Key" text boxes with a deployment picker dropdown (None/Standalone, Cloudflare Worker, Bridge LAN, Bridge Docker). Conditional fields per service. Destructive transition warning dialog with export button when switching from local-only to remote.
7. 🔜 **Destructive transition warning dialog** with export button.
