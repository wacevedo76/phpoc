...
## Known Issues
- **Web suite — 7 pre-existing failing test files (discovered 2026-08-31, unrelated to Commonplace Phase 3):** `cross_client_web_test.mjs` (4), `import_entries_test.mjs` (1 J7), `ledger_sync_test.mjs` (2), `reauth_genesis_mismatch_test.mjs` (2), `settings_genesis_test.mjs` (3), `sync_service_test.mjs` (3), `unlock_performance_regression_test.mjs` (3). All in sync/genesis-compatibility/UI/import domains. Commonplace Phase 3 source changes are purely additive (verified via `git diff`) and do not touch these; root causes not yet investigated.
