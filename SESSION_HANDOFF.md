...
- **🔴 HIGH — C-2 Full Seed Replacement (new seed re-key)** — plan at `docs/planning/flutter/SEED_REKEY_C2_PHASE1.md` (Phase 1 blueprint, 34 assertions R/B/M/P/S). The only capability that truly nullifies the **leaked seed** (in git history): mint a fresh random seed → full re-key of vault + chain + remote/R2 + device cookie under the new root, with backup + two-secret confirm. **Option (a) CONFIRMED at Phase 2** (new seed = new raw MK; key_version unchanged; NO new ledger-block metadata — re-key meta lives in AppPreferences only, not chain schema). **Phase 2 RED DONE (2026-08-22):** `RekeyService` skeleton (`lib/services/rekey_service.dart`, methods throw `UnimplementedError`) + `test/services/rekey_service_test.dart` (Groups R/B/M/P; service RED +3/-25) + `test/features/settings_screen_test.dart` Group S (UI RED +23/-6, no pre-existing regressions). Blueprint test M1 adapted→M1a (key_version preserved); M5/P4/P6 deferred placeholders (depend on Commonplace + real pull/push transport). **Next: Phase 3 (GREEN)** implement `RekeyService` + `rekeyServiceProvider` + Settings re-key UI. Also: wire the unreachable `ph rotate-keys` into `main.py` as parity/escape-hatch precedent.
