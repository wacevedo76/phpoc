...
- **TTL cookie bypassed for local-only ledgers** — `StagingService.check_and_sync()` returned `READY` immediately when no remote transport was configured (`_remote is None`), skipping the device cookie TTL check entirely. Read commands (`ph list`/`view`/`tags`) and write commands never prompted for a passphrase on TTL expiry in local-only mode because the session cache (`/dev/shm/phpoc_session`) had no TTL. Fix: `check_and_sync()` now checks `DeviceCookie.is_valid_locally()` even for local-only, returning `REAUTH_NEEDED` on expiry. `_reconcile_and_claim()` handles local-only via new `DeviceCookie.create_local()`. `ph login` and `ph recover` handlers create local cookies. Files: `domain/cookie/device_cookie.py` (+`create_local()`), `domain/staging/service.py` (local-only TTL gate in `check_and_sync()` + `_reconcile_and_claim()`), `main.py` (login/recover cookie creation). (2026-06-27)
