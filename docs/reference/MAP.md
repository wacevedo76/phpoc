...
| `phpoc-web/src/components/layout/BookSwitcher.jsx` | 🟢 GREEN | **NEW (Commonplace Slice 2 — Book Switcher, Phase 3)** — persistent title bar with pull-down menu (both books); `useState` seeded from `Book.fromKey(getBookMode())`; select → `setBookMode`. Rendered once by `AppLayout` above `.app-content`. |
