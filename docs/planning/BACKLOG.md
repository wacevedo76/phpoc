...
- **Phase D:** ✅ cross-client verify (Web↔Flutter hermetic matrix GREEN 18/18 each direction; old-seed device can't decrypt) + **live R2 E2E ✅ GREEN (2026-08-28)** (`tests/test_c2_live_r2.py` re-keys the real test ledger via WASM, old-seed nullified), spec/format doc pass (PHPSPEC §2.10), ROADMAP/WEB_ROADMAP/MAP/CHANGELOG updates. CLI↔client E2E now **un-gated by CLI Phase A (2026-08-29)**. **CLI leg started (2026-08-29):** `docs/planning/C2_CLI_CLIENT_VERIFY_PHASE1.md` — Phase 1 blueprint (48 assertions, groups A–E); 5 divergences documented (R1 CLI `key_version` bump vs Web/Flutter option (a) raw-seed; R2 raw-seed `key_version=0` test ledger vs CLI default-1 gate; R3 web verify raw-MK convention; R4 `key_version=1` raw-seed-vs-HMAC ambiguity; R5 CLI multi-version lookup starts at v=1). **Decision resolved: option (a) raw-seed re-key (ADR-032).** **Phase 2 (RED) DONE (2026-08-29):** Python 22 RED/12 guard-green, Flutter 9 RED/2 skip, live R2 skip offline. **R6 (new):** `identity_pub_key` convention — Python raw-bytes vs Web/Flutter hex-string. **Phase 3 (GREEN) DONE (2026-08-29):** option (a) raw-seed re-key implemented; hermetic matrix GREEN all 4 directions (`test_c2_cli_client_verify.py` 34/34, `test_rekey_seed.py` 34/34, Flutter `c2_cli_verify_test.dart` 9/2skip, `test_i01_*` 56); R1–R6 resolved (R6 canonical = raw-bytes `identity_pub_key` — Python correct, Web/Flutter hex-string divergence was a follow-up, resolved 2026-08-31); legacy `block_hash`-on-day-blocks fixed. **Phase 4 (REFACTOR + Group E docs) ✅ DONE (2026-08-29):** `_rebuild_rekeyed_blocks` re-seals via canonical `LedgerChain._hash_key_for_block`; `_content_hash_map` harness DRY; PHPSPEC §2.10 harness citation + Group E docs updated. **Live R2 E2E ✅ GREEN (2026-08-29):** canonical test ledger restored to R2 (genesis `e718daf3…`, 31 blocks) + forward-leg `renew_seed()` `transport=None` fix (isolated push); `test_c2_cli_client_live_r2.py` 2/2 + `test_c2_live_r2.py` 1/1. **Follow-up `identity_pub_key` raw-bytes parity ✅ DONE (2026-08-31):** `C2_IDENTITY_PUB_KEY_RAW_BYTES_PHASE1.md` Phase 3 (GREEN) — raw-bytes `identity_pub_key` in Rust/WASM/FRB + Web/Flutter wrappers, fixtures regenerated `271a413b…`→`9a2db2e2…`.
