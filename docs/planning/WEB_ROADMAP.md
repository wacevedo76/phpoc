...
| 38 | **Remote Sync Wiring — phpoc-web ↔ Cloudflare Worker** — Dual-backend model (local IndexedDB + remote HttpTransport), device UUID persistence (random UUID4 per device, not HMAC-derived), ledger merge strategy (GREEN with input validation + ordering tests), Settings UI with genesis compatibility gate. | 🔴 In progress | — | Jun 18-19 2026 |
