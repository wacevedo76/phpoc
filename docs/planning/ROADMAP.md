...
| **Onboarding/ReAuth Speedup** — Hash-index based genesis check replacing full block pulls. 4-phase TDD plan at `docs/planning/ONBOARDING_UNLOCK_REAUTH_SPEEDUP_STRATEGY.md`. | 🔜 | High | Cuts genesis check from ~21s to ~0.1s (210×). Phase 1: test identification in progress. |
