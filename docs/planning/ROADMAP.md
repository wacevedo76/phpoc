...
|---|---|---|---|
| **Commonplace Book** — A personal thematic library (title, `tags`, `entry`, optional ad-hoc k/v, **no `comment`**) stored as a **separate sealed `commonplace.json`** chain sharing the main ledger's seed→MK, with its own genesis block (ADR-031). All content encrypted; append-only day-grouped sealed commits; staging→commit workflow (D11). Flutter first, then Web, then CLI. | 🟡 In progress | Medium | Phase 1 blueprint: `docs/planning/flutter/COMMONPLACE_BOOK_PHASE1.md` (55 assertions, groups A–F). ADR-031 documents the decision. Phase 3 (GREEN): 55/55 tests pass for the Flutter chain/engine/storage engine (2026-08-21). Phase 4 (REFACTOR): done — shared `SealableChain` mixin deduped with the ledger layer; 349/349 ledger-layer tests GREEN (2026-08-21). UI-wiring first step ✅ (2026-08-21): shell-level **Book Switcher** bar (`PH Ledger` ↔ `PH Commonplace Book`) rendered above each page in `AppScaffold`, selection persisted in `AppPreferences` (`book_mode`); 13/13 tests GREEN, no regressions. **UI-wiring screen surface ✅ (2026-08-23):** `CommonplaceService` + `CommonplaceScreen`/`AddEntryBottomSheet`/`TopicIndex` + `commonplaceServiceProvider` + `AppScaffold` content-swap by book; 40/40 GREEN, full suite +2050/0 fails (Phase 1 doc: `COMMONPLACE_BOOK_UI_PHASE1.md`). **UI-wiring Phase 4 (REFACTOR) ✅ (2026-08-23):** removed dead code in `_refresh`, documented + extracted the identityless genesis bootstrap (`_ensureBookBootstrap`), consolidated the `store` persistence cast + provider store construction; 40/40 GREEN retained, full suite +2050/0. **Settings surface ✅ (2026-08-24, 4-phase TDD):** `CommonplaceSettingsScreen` (verify/backup/restore/clear-all/re-key/push-stub/shared-worker/per-book-theme) routed on `/settings` in Commonplace mode; 46/46 GREEN (34 widget + 12 service), full suite +2096/0, analyze 0; Phase 4 REFACTOR done (DRY'd `ThemeVariantNotifier`, extracted `RekeyService._rekeyCommonplace`, removed `dynamic` transport params + dead lock code). Phase 1 doc: `COMMONPLACE_BOOK_SETTINGS_PHASE1.md`.

**Sub-slices (tracked in `BACKLOG.md`):** 1️⃣ UI wiring — **complete ✅ (2026-08-23, 4-phase TDD):** `CommonplaceService` + `CommonplaceScreen`/`AddEntryBottomSheet`/`TopicIndex` + provider + `AppScaffold` content-swap; 40/40 GREEN + Phase 4 REFACTOR done (`COMMONPLACE_BOOK_UI_PHASE1.md`). 2️⃣ Remote sync — same Worker, new R2 path `commonplace/...` + MK-derived device cookie (⏸️ pending). 3️⃣ Shared key-rotation extension — ADR-026 re-encrypts Commonplace chain(s) (**Flutter 🔜 Phase 1 blueprint** `docs/planning/flutter/COMMONPLACE_BOOK_KEY_ROTATION_PHASE1.md` (59 assertions, groups A–E) — Flutter ADR-026 rotation not yet implemented (the Settings slice shipped C-2 **seed replacement**, not rotation); Web/CLI ⏸️ pending). 4️⃣ Tag-search blind index — encrypted, MK-derived (⏸️ deferred; decrypt-and-scan initially). 5️⃣ Parity ports — Web then CLI (🟡 Web in progress: Slice 1 chain/engine/storage ✅ + Slice 2 Book Switcher ✅, Slices 3–6 pending — `COMMONPLACE_BOOK_WEB_ROADMAP.md`; CLI ⏸️ pending). |
...
| Item | Status | Priority | Notes |
|---|---|---|---|
| **Adaptive landscape layout** — shell taskbar relocates to a left `NavigationRail` in landscape (bottom `NavigationBar` in portrait) so the reduced vertical space isn't consumed by a bottom bar | 🟡 In progress | Medium | **Slice 1 ✅ (2026-09-04):** `AppScaffold` is orientation-aware (`MediaQuery.orientationOf`); the four destinations are deduped into a shared `_ShellTab` list; added `setSurfaceSize()` test helper + forced portrait on 4 legacy `NavigationBar` assertions + 3 new landscape tests (I9–I11). **Slice 2 ✅ (2026-09-04):** History screen splits into side-by-side calendar (left) and activities (right) panes at 50/50 width in landscape (`_buildCalendar`/`_buildFilterBar`/`_buildListOrEmpty` helpers); empty-state falls back to full-width; landscape activities list shows an always-visible `Scrollbar` (`_listScrollController` + `thumbVisibility`); 4 adaptive-layout tests (L1–L4). Also enabled mouse-wheel/trackpad scrolling app-wide via a custom `AppScrollBehavior` (`MaterialApp.scrollBehavior` + `dragDevices` includes `mouse`) — Flutter's Android default ignores non-touch input, which made lists appear unscrollable on the emulator. Landscape nav rail slimmed: the outer `SafeArea` no longer pads left/right (`left: false, right: false`) — in landscape `padding.right` is the system navigation-bar inset (~48dp), which was opening a wide empty gutter to the right of the rail; the left cutout is handled by `NavigationRail`'s internal `SafeArea`. Labels kept visible under icons (`labelType: all`, `minWidth: 64`); tests I11/I12 updated. Additional landscape UX issues pending. |
...
| **Wipe local ledger data from unlock screen** | ✅ | Low | DESTRUCTIVE — permanently deletes ALL local data (ledger blocks + entries, staging, seed, credentials, MK) and returns to a fresh onboarding state. Cloud (R2) data is NOT affected. Implemented in Flutter (`authService.wipeLedger()` from `unlock_screen.dart`, error-colored `OutlinedButton` + confirmation dialog). **Web parity (2026-08-22):** `phpoc-web` `wipeLedger()` in `DevModeContext.jsx` clears the local IndexedDB backend + localStorage worker creds + MK, then navigates to landing (hasExistingData=false → onboarding); wired into `AuthScreen.jsx` as a destructive `auth-btn--wipe` button below Unlock with a confirmation dialog (mirrors Flutter). See `docs/planning/WEB_ROADMAP.md` Build 63. Source parity: `wipeLedger()` ↔ Flutter `AuthService.wipeLedger()`. |
...
| **Full Seed Replacement (Re-key)** | ✅ per-client | — | **C-2**: mint a fresh random seed, re-encrypt the entire ledger (vault, every block `_enc`, genesis seed, remote/R2, device cookie) under the new Master Key so a leaked/compromised old seed is nullified. **Flutter ✅ COMPLETE** (4-phase TDD, 2026-08-22): 28/28 `rekey_service_test.dart` + 6/6 Settings Group S, suite 2010/2010, analyze 0; wired `ph rotate-keys`(+`--full`) into `main.py`. **Web + CLI ports 🔜 PLANNED (HIGH priority)** — plan/roadmap: `docs/planning/C2_SEED_REKEY_WEB_CLI_ROADMAP.md`; CLI `--renew-seed` + `ph rekey-seed`, web `RekeyService` + Settings Security & Recovery UI (backlog HIGH). **Web ✅ 4-phase TDD COMPLETE (2026-08-24):** `src/services/rekey_service.js` (`RekeyService`, 28/28 `rekey_service_web_test.mjs`) + Settings Security & Recovery UI (6/6 `rekey_settings_web.test.mjs`) + `DevModeContext.rekey(...)`; Phase 4 REFACTOR extracted named phase helpers. **Cross-client verify (Phase D) ✅ Phase 4 (REFACTOR) COMPLETE (2026-08-28):** hermetic matrix GREEN both directions (Web↔Flutter, old-seed-fail) — `docs/planning/C2_CROSS_CLIENT_VERIFY_PHASE1.md`; 4 divergences fixed (content_hash kept-suffix, genesis identity, canonical re-key, PDK salt). Phase 4: harness DRY'd, spec pass (PHPSPEC §2.10), docs (WEB_ROADMAP/BACKLOG/C2 roadmap). **Live R2 E2E ✅ GREEN (2026-08-28):** `tests/test_c2_live_r2.py` — real test ledger re-keyed via WASM, old-seed nullified; Python `content_hash` divergence fixed to §5.5/§6.1 KEEP. **Flutter `computeContentHash` strip divergence ✅ FIXED (4-phase TDD, 2026-08-28):** all three compute impls KEEP `_enc` + `jsonSort`; full suite `+2132`/0. **CLI ✅ 4-phase TDD COMPLETE (2026-08-29):** `renew_seed`/`mint_new_seed`/`seed_fingerprint` + `ph rekey-seed` in `main.py`; 34/34 `test_rekey_seed.py` GREEN; Phase 4 REFACTOR DRY'd the per-`_enc` re-key loop (`_enc_fields`/`_decrypt_crypto_for_version`/`_prevalidate_entries_decryptable`/`_reencrypt_entry_data`) + split `renew_seed` into `_prepare_rekey`/`_rebuild_rekeyed_blocks`/`_persist_rekeyed_state`/`_push_rekeyed_state`; full suite 2649 pass/1 skip/0 fail. Transport adapter gap RESOLVED (2026-08-29): re-key remote push routes through `RemoteStagingSync`/`RemoteLedgerSync` on the real `AbstractStagingTransport.push(path,data)`/`pull(path)` contract; P1–P6 + I6 tests updated. **CLI↔client cross-client verify ✅ Phase 3 (GREEN) (2026-08-29):** option (a) raw-seed re-key (`_get_current_key_version` default-0, `_make_multi_version_mk_lookup` v=0, no `key_version` bump); hermetic matrix GREEN all 4 directions (`test_c2_cli_client_verify.py` 34/34, Flutter `c2_cli_verify_test.dart` 9/2skip); R1–R6 resolved (R6 canonical = raw-bytes `identity_pub_key`); legacy `block_hash`-on-day-blocks fixed. **Phase 4 (REFACTOR + docs) ✅ DONE (2026-08-29):** harness DRY'd (canonical `_hash_key_for_block` re-seal + `_content_hash_map` helper), PHPSPEC §2.10 harness citation updated, Group E docs applied. **Live R2 E2E ✅ GREEN (2026-08-29):** canonical test ledger restored to R2 (genesis `e718daf3…`); forward-leg `renew_seed()` fixed to `transport=None` (isolated push — no canonical `ledger/blocks/` overwrite); `test_c2_cli_client_live_r2.py` 2/2 + `test_c2_live_r2.py` 1/1. **R6 raw-bytes `identity_pub_key` parity ✅ DONE (2026-08-31):** `C2_IDENTITY_PUB_KEY_RAW_BYTES_PHASE1.md` Phase 3 (GREEN) — Web/Flutter now hash raw bytes, fixtures `271a413b…`→`9a2db2e2…`. |
