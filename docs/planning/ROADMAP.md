...
**Sub-slices (tracked in `BACKLOG.md`):** 1️⃣ UI wiring — **complete ✅ (2026-08-23, 4-phase TDD):** `CommonplaceService` + `CommonplaceScreen`/`AddEntryBottomSheet`/`TopicIndex` + provider + `AppScaffold` content-swap; 40/40 GREEN + Phase 4 REFACTOR done (`COMMONPLACE_BOOK_UI_PHASE1.md`). 2️⃣ Remote sync — same Worker, new R2 path `commonplace/...` + MK-derived device cookie (⏸️ pending). 3️⃣ Shared key-rotation extension — ADR-026 re-encrypts Commonplace chain(s) (**Flutter done ✅ via Settings slice 2026-08-24**; Web/CLI ⏸️ pending). 4️⃣ Tag-search blind index — encrypted, MK-derived (⏸️ deferred; decrypt-and-scan initially). 5️⃣ Parity ports — Web then CLI (⏸️ pending). |
