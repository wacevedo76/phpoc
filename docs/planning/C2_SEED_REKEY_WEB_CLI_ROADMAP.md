...
> **Status:** 🟢 **COMPLETE — Web (4-phase TDD, 2026-08-24), Phase D cross-client Web↔Flutter + spec + docs (2026-08-28), CLI Phase A (4-phase TDD, 2026-08-29), and the CLI↔client verify leg (4-phase TDD, 2026-08-29) all DONE.** Cross-client port. See `docs/planning/BACKLOG.md`. Web: Phase 1 blueprint `docs/planning/C2_SEED_REKEY_WEB_PHASE1.md` (34 assertions R/B/M/P/S) → Phase 2 RED (28 Node + 6 Vitest) → Phase 3 GREEN (`src/services/rekey_service.js` 28/28 + Settings Security & Recovery UI 6/6 + `DevModeContext.rekey`) → **Phase 4 REFACTOR ✅** (extracted `rekey()` into named phase helpers). Phase D hermetic matrix GREEN (Web re-key→Flutter verify A7–A11, Flutter re-key→Web verify B7–B11, crypto invariants C — 18/18 each direction). **Live R2 E2E ✅ GREEN (2026-08-28):** real test ledger re-keyed via WASM `RekeyService`, old-seed nullified (`tests/test_c2_live_r2.py`). **CLI Phase A ✅ 4-phase TDD COMPLETE (2026-08-29):** `renew_seed`/`mint_new_seed`/`seed_fingerprint` + `ph rekey-seed` in `main.py`; 34/34 `test_rekey_seed.py` GREEN; Phase 4 REFACTOR DRY'd per-`_enc` loop + split `renew_seed` into phase helpers; full Python suite 2649 pass/1 skip/0 fail. **Transport adapter gap RESOLVED (2026-08-29):** the re-key remote push now routes through the real `AbstractStagingTransport.push(path,data)`/`pull(path)` contract — cookie via `transport.push(REMOTE_COOKIE_PATH, …)`, staging via `RemoteStagingSync.push`, ledger via `RemoteLedgerSync.push_blocks`/`push_hash_index`/`push_index`; P1–P6 (and rotation I6) tests updated to the real contract. **CLI↔client leg ✅ 4-phase TDD COMPLETE (2026-08-29):** `docs/planning/C2_CLI_CLIENT_VERIFY_PHASE1.md` — option (a) raw-seed re-key implemented (`_get_current_key_version` default-0, `_make_multi_version_mk_lookup` v=0, `_prepare_rekey` raw-seed MK, no `key_version` bump); hermetic matrix GREEN all 4 directions (`test_c2_cli_client_verify.py` 34/34, Flutter `c2_cli_verify_test.dart` 9/2skip); R1–R6 resolved (R6 canonical = raw-bytes `identity_pub_key`, Python correct; Web/Flutter hex-string divergence **RESOLVED 2026-08-31** via `C2_IDENTITY_PUB_KEY_RAW_BYTES_PHASE1.md` Phase 3). Phase 4 (REFACTOR + Group E docs) DONE; live R2 E2E remains live-only.
...
- **[D1] Cross-client re-key verification** ✅ — hermetic matrix GREEN both directions (Web re-key→Flutter verify A7–A11; Flutter re-key→Web verify B7–B11; crypto invariants C), 18/18 pass + 2 live-only skip per harness. A device holding the OLD seed/MK no longer decrypts (leak nullification proof, A11/B11/C7). **Live R2 E2E ✅ (2026-08-28):** `tests/test_c2_live_r2.py` re-keys the real test ledger via WASM `RekeyService` against the live Worker and proves old-seed nullification end-to-end. **CLI↔client E2E (`ph rekey-seed` → web/Flutter pull+verify) ✅ 4-phase TDD COMPLETE (2026-08-29):** hermetic matrix GREEN all 4 directions (`C2_CLI_CLIENT_VERIFY_PHASE1.md`); Phase 4 REFACTOR (canonical `_hash_key_for_block` re-seal + `_content_hash_map` harness DRY) + Group E docs DONE; live R2 E2E (`tests/test_c2_cli_client_live_r2.py`) remains live-only (needs clean live Worker state).
